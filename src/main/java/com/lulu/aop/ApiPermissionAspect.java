...
public class ApiPermissionAspect {
    ...
    public void checkApiPermission(JoinPoint joinPoint, ApiPermission apiPermission) {
        ...
        log.info("獲取當前使用者的頁面權限");
        log.info(currentUserPermission.toString());
        log.info("ApiPermission apiPermission");
        log.info(String.valueOf(apiPermission));
         ...
         * 比如使用者有權限訪問 A 頁面，currentUserPermission 的值可能是 "A"
         *
         ...
         */
//        boolean hasPermission = Arrays.stream(apiPermission.value())Session ID in ApiPermissionAspect: 051744A708E57C9DD2DCBC2E8C308862
//                .anyMatch(permission -> permission.equals(currentUserPermission));
